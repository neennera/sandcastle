<script lang="ts" context="module">
	...
	let name = '';
	let email = '';
	let castleType: 'lotus' | 'layer' | 'octagonal' | 'flora' | '' = ''; // Default value

	let toastVisible = false; // State to control the visibility of the toast
	let toastMessage = '';

	const details: Record<'lotus' | 'layer' | 'octagonal' | 'flora', string> = {
						...
						{#each castleOptions as option}
							<div class="flex w-[25%] flex-col items-center space-y-1 text-[#8D7878]">
								<button
									type="button"
									on:click={() => {
										castleType = option as 'lotus' | 'layer' | 'octagonal' | 'flora'; // Set the selected castle type
										toastVisible = true;
										toastMessage = details[castleType]; // Set the toast message based on the selected option
									}}
