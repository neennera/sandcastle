...
<div class="absolute bottom-0 z-15 bg-[#f3e7ce] border-[2px] border-[#e6d1a3] h-[10%] w-full rounded-[20px]" id="deco">
    <button
      class="absolute top-[-25px] left-[20%] h-12 w-[60%] text-[#c28d1e] text-2xl font-semibold rounded-[20px] bg-[#f9d790] border-[#efc368] hover:border-[2px]"
      on:click={() => {openD('#deco'); isOpen=true;}}
    >
      ตกแต่งเพิ่ม
    </button>
    {#if isOpen}
    <div class="w-[90%] h-[90%] flex items-center justify-center mt-[4%] ml-[5%]">
      <div class="grid grid-cols-2 grid-rows-3 w-[80%] h-full ">
        {#each decoprops as deco, index (index)}
        <div
            class="w-24 h-24 flex items-center justify-center rounded-lg hover:scale-120 transition-transform duration-200 ease-in-out"
      ...
      <div class="w-full h-full text-[#bd7878] justify-center items-center text-xl font-semibold flex flex-col ml-4 pl-4">
        {#if clickedIndex !== null}
        <h1 class="font-bold text-2xl">{decoprops[clickedIndex].name}</h1>
        <h2>{decoprops[clickedIndex].detail}</h2>
