...
>
	<SvelteToast />
	<button
		class="absolute top-[-25px] left-[20%] h-[5vh] w-[60%] rounded-[20px] border-[#efc368] bg-[#f9d790] text-xl sm:text-3xl font-semibold text-[#c28d1e] hover:border-[2px]"
		on:click={() => {
			openD('#deco');
			isOpen = true;
		}}
	...
	<div class="flex h-full w-full flex-col mt-4">
		{#if isOpen}
			{#if !isFormPage}
				<!-- Selection Page -->
				<div class="mt-[4%] ml-[5%] flex h-[70%] w-[90%] items-center justify-center">
					<div class="grid h-full w-[80%] grid-cols-2 grid-rows-3">
						{#each decoprops as deco, index (index)}
							<div
								class="flex h-20 w-20 sm:h-32 sm:w-32 items-center justify-center rounded-lg transition-transform duration-200 ease-in-out hover:scale-120"
					...
					<div
						class="mt-4 ml-4 flex h-full w-full flex-col items-center justify-center pl-4 text-xl sm:text-2xl font-semibold text-[#bd7878]"
					>
						{#if clickedIndex !== null}
							<h1 class="text-lg sm:text-3xl font-bold">{decoprops[clickedIndex].name}</h1>
							<h2 class="text-sm sm:text-xl">{decoprops[clickedIndex].detail}</h2>

							<h1 class="mt-2 text-sm sm:text-xl font-medium">
								{decoprops[clickedIndex].description}
							</h1>

							<button
								class="btn-secondary mt-4 mb-4 self-center text-sm sm:text-xl"
			...
			{:else}
				<!-- Form Page -->
				<div class="flex flex-col h-full items-center justify-start px-4 py-2 mt-0 sm:mt-4">
					<input
						type="text"
						placeholder="ชื่อผู้ส่ง"
						bind:value={sender_name}
						class="mt-4 w-full rounded-[20px] text-sm border-[2px] border-[#8d7878] bg-[#fdfde6] p-2 focus:border-[#e3d1a0] focus:outline-none"
					/>
					<textarea
						placeholder="ข้อความอวยพร"
						bind:value={wishing_text}
						class="mt-4 w-full h-[30%] rounded-[20px] text-sm border-[2px] border-[#8d7878] bg-[#fdfde6] p-2 focus:border-[#e3d1a0] focus:outline-none"
					></textarea>
					<div class="mt-4 flex w-full justify-between gap-4">
						<button
							class="btn-secondary w-[50%] h-[45px] sm:h-[60px] text-sm sm:text-lg"
